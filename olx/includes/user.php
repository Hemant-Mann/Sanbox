...
class User extends DatabaseObject {
	...
	public function save() {
		...
		if(isset($this->id)) {
			...
			$sql1 .= "WHERE phone = '{$safe_phone}' ";
			$result_set = $this->find_by_sql($sql1);
			if(!empty($result_set)) {
				$phone_exists = $database->fetch_array($result_set);
				if($phone_exists["phone"] === $safe_phone) {
			...
			$sql1 = "SELECT * FROM users ";
			$sql1 .= "WHERE email = '{$safe_email}' ";
			$result_set = $this->find_by_sql($sql1);
			if(!empty($result_set)) {
				$email_exists = $database->fetch_array($result_set);
				if($email_exists["email"] === $safe_email) {
			...
			$sql2 .= " WHERE username = '{$safe_username}' ";
			$result_set = $this->find_by_sql($sql2);
			if(!empty($result_set)) {
				$username_exists = $database->fetch_array($result_set);
				if($username_exists["username"] === $safe_username) {
